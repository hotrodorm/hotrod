...
@Component
public class ReportingDAO implements ApplicationContextAware {
  ...
  public List<Row> find2() {
    Parameters params = this.dyn.newParameters();
    MapRowReader rr = new MapRowReader();
    PreparedSelectQuery<Row> preparedQuery = this.select1.prepare(params, rr);
    logQuery(preparedQuery);
    Connection conn = null;
    try {
      conn = DataSourceUtils.getConnection(this.dataSource);
      List<Row> rows = preparedQuery.execute(conn);
      return rows;
    } catch (SQLException e) {
      throw new PersistenceException(e);
    } finally {
