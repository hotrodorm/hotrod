-- create table person (
--   id bigint identity primary key not null,
--   photo varbinary(1000),
--   contract varchar(max),
--   name varchar(20)
-- );

--insert into person (id, photo) values 
--  (1, 
--);


delete from person;

insert into person (id, photo, contract, name) values (1, 0x00112233, 'This is a job contract', 'Alice');
insert into person (id, photo, contract, name) values (2, 0x12345678, 'This is a job contract for Archie', 'Archie');
insert into person (id, photo, contract, name) values (3, 0x89504e47, 'This is a job contract for Anne', 'Anne');
insert into person (id, photo, contract, name) values (4, 0x00112233, 'This is a job contract', 'Alan');



  