...
//  private LiveSQL sql2;
  ...
  public CommandLineRunner commandLineRunner(ApplicationContext ctx) {
    return args -> {
      ...
      logBeans(ctx, DataSource.class);
      logBeans(ctx, LiveSQL.class);
      logBeans(ctx, LiveSQLDialect.class);

      System.out.println("this.accountDAO=" + this.accountDAO);
      searching();
      System.out.println("[ Example complete ]");
    };
  }

  private void logBeans(ApplicationContext ctx, Class<?> c) {
    String[] beans = ctx.getBeanNamesForType(c);
    System.out.println("* Found " + beans.length + " beans of class -- " + c.getName());
    for (String b : beans) {
