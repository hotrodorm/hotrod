...
Torcs records to all query executions in the application and consolidates statistical data
to detect slow running queries.

Even though it's a part of HotRod, Torcs can be used with or without the HotRod ORM, in any Spring or SpringBoot application.

Torcs does not replace the database statistic information. It's local to the application instance, and does not see the queries running in other instances, or queries executed by other applications that may slow down the database for external causes.
