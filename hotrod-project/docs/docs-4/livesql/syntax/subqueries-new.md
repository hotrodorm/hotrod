...
The initial types of subqueries implemented in LiveSQL:
...
produce real-life, complex queries.
It includes:

- **Scalar Subqueries**: Scalar Subqueries are embedded subqueries in the select list that return one value and at most one row. They
...
- **Common Table Expressions (CTEs)**: Both types of Common Table Expressions &mdash; plain and recursive &mdash; are implemented.

- **Lateral Joins**: Inner and outer lateral joins are implemented. Lateral joins support is only available in the databases that support them. LiveSQL support lateral joins in the following databases:
