...
can be plain or correlated to the main query. Assymmetric operators combine the traditional operators `=`, `<>`, `<`, `>`, `<=`, `>=` by appending the extra clauses `ALL` or `ANY` to them.
...
There's also an alternative method `asUnion()` that implements recursive CTEs using the `UNION` operator rather than the `UNION ALL` operator. This variant is only supported by the PostgreSQL and MariaDB databases.
