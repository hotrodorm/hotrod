...
The inclusion of table expressions and CTEs in version 4.1 change dramatically the range of queries can be expressed in LiveSQL. The following types of subqueries are now implemented in LiveSQL.

### LiveSQL 3.0

LiveSQL 3.0 implements subqueries limited to a few specific cases. They are available for:
...
(with any value or even nulls) is generated by subquery. This subquery can be plain or correlated
to the main query.
...
The addition of the following types of subqueries enhances the syntax of LiveSQL to
address more complex use cases. It includes:

- **Scalar Subqueries**: Scalar Subqueries are subqueries whose result take the place of a single
value in the outer query. They are typically used to produce values in the select list (or
in other parts of the query) that are computed by a query that needs to access system properties or data from related or unrelated tables.
