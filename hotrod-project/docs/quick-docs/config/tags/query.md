...
A `<query>` is intended to execute queries that do not return a result set. That is it's not intended to execute
a `SELECT` SQL statement. To return a result set use the `<select>` tag instead.
...
towards high performance SQL and query optimization.
