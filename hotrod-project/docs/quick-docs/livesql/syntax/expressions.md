...
| CASE WHEN a THEN b END<hr/>CASE WHEN a THEN b ELSE e END<hr/>CASE WHEN a THEN b WHEN c THEN d ELSE e END | CASE | `sql.caseWhen(<a>, <b>).end()`<hr/>`sql.caseWhen(<a>, <b>).elseValue(e).end()`<hr/>`sql.caseWhen(<a>, <b>).when(<c>, <d>).elseValue(e).end()` |
