...
| CASE WHEN a THEN b END<hr/>CASE WHEN a THEN b ELSE e END<br/>CASE WHEN a THEN b WHEN c THEN d ELSE e END | CASE | `sql.caseWhen(<a>, <b>).end()`<br/>`sql.caseWhen(<a>, <b>).elseValue(e).end()`<br/>`sql.caseWhen(<a>, <b>).when(<c>, <d>).elseValue(e).end()` |
