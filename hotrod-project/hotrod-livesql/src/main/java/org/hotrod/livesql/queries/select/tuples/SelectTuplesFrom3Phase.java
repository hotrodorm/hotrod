...
public class SelectTuplesFrom3Phase<A, B, C> {
  ...
  public SelectWherePhase<Tuple3<A, B, C>> where(final GeneralBooleanExpression predicate) {
    return SHelper.getSelectWherePhase(this.metadata.getContext(), new TuplesSelect<Tuple3<A, B, C>>(this.metadata),
        predicate);
  }

  public SelectGroupByPhase<Tuple3<A, B, C>> groupBy(final ComparableExpression... columns) {
    return SHelper.getSelectGroupByPhase(this.metadata.getContext(), new TuplesSelect<Tuple3<A, B, C>>(this.metadata),
        columns);
  }

  public LockableSelectOrderByPhase<Tuple3<A, B, C>> orderBy(final OrderingTerm... orderingTerms) {
    return SHelper.getSelectOrderByPhase(this.metadata.getContext(), new TuplesSelect<Tuple3<A, B, C>>(this.metadata),
        orderingTerms);
  }

  public LockableSelectOffsetPhase<Tuple3<A, B, C>> offset(final int offset) {
    return SHelper.getSelectOffsetPhase(this.metadata.getContext(), new TuplesSelect<Tuple3<A, B, C>>(this.metadata),
        offset);
  }

  public LockableSelectLimitPhase<Tuple3<A, B, C>> limit(final int limit) {
    return SHelper.getSelectLimitPhase(this.metadata.getContext(), new TuplesSelect<Tuple3<A, B, C>>(this.metadata),
        limit);
  }
  ...
  public List<Tuple3<A, B, C>> execute() {
    CombinedSelectObject<Tuple3<A, B, C>> combined = new CombinedSelectObject<>(new TuplesSelect<>(this.metadata));
