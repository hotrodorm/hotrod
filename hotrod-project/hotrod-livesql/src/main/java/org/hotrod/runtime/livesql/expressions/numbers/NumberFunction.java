...
public abstract class NumberFunction extends NumberExpression {
  ...
  protected NumberFunction(final String name, final List<Expression> parameters) {
    super(Expression.PRECEDENCE_FUNCTION);
    if (SUtil.isEmpty(name)) {
      throw new InvalidFunctionException("The function name cannot be empty");
    }
  ...
  protected NumberFunction(final String name, final Stream<Expression> parameters) {
    super(Expression.PRECEDENCE_FUNCTION);
    if (SUtil.isEmpty(name)) {
      throw new InvalidFunctionException("The function name cannot be empty");
    }
