...
public abstract class StringFunction extends StringExpression {
  ...
  protected StringFunction(final String name, final boolean includeParenthesis, final Expression... parameters) {
    super(Expression.PRECEDENCE_FUNCTION);
    if (SUtil.isEmpty(name)) {
      throw new InvalidFunctionException("The function name cannot be empty");
    }
  ...
  protected StringFunction(final String name, final String qualifier, final boolean includeParenthesis,
