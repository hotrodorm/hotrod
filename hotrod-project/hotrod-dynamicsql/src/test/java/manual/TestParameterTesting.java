...
//    DynamicSelectQuery q = b.literal("SELECT * FROM t WHERE a >= ").parameter("id").endSelectQuery();
    DynamicSelectQuery q = b.literal("SELECT * FROM t WHERE a in ") //
        .foreach("v", "codes", "(", ", ", ")", b.parameter("v").end() //
        ). //
        endSelectQuery();

    ParameterContext ctx = b.newParameterContext();
    ctx.add("id", 12);
    ctx.add("name", "ghi");
    ctx.add("cost", 105.43);
    ctx.add("codes", codes);
...
//    DynamicSelectQuery q = b.literal("SELECT * FROM t WHERE a >= ").parameter("id").endSelectQuery();
    DynamicSelectQuery q = b.literal("SELECT a FROM t WHERE a in ") //
        .foreach("v", "codes", "(", ", ", ")", b.parameter("v").end() //
        ). //
        endSelectQuery();
    ...
    try (Connection conn = DriverManager
        .getConnection("jdbc:h2:mem:EXAMPLEDB;INIT=runscript from './schema.sql';DB_CLOSE_DELAY=-1", "", "")) {
      System.out.println("-- Connected --");
      List<Integer> rows = pq.execute(conn);
      rows.forEach(r -> System.out.println("r: " + r));
    }
