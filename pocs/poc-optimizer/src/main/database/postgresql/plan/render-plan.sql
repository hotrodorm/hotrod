...
n (xid, parent_xid, child_number, o) as (
  select '1', '0', 1, o from (select t -> 0 -> 'Plan' as o from plan) x
 union all
  select n.xid || '.' || s.ord, n.xid, s.ord::int, s.o
...
microformat (less_than, available, divisor, tformat, append) as (
  ...
  union all select 1000000000, 1, 1000000000, 'FM999990.00', 'G'
),
cost_mf as (select * from microformat where (select max(cost) from p) < less_than or available = 1 order by less_than limit 1),
rows_mf as (select * from microformat where (select max(rows) from p) < less_than or available = 1 order by less_than limit 1),
icon as (
  select '<font point-size="10" color="#00bb34">&#127352;</font>' as idx,
         '<font point-size="10" color="#0000ff">&#127351;</font>' as heap,
         '<font point-size="10" color="#ff8c00">&#127349;</font>' as filter
),
g (section, line, rendered) as (
  select 1, 1, -- 1. Opening
    'digraph p1 { rankdir=BT; ranksep=0.25; graph [fontname = "helvetica", fontsize=9, bgcolor="#f0f0f0"]; node [fontname = "helvetica", fontsize = 9]; edge [fontname = "helvetica", fontsize = 9]; labelloc="t";'||
    ' label="SQL Execution Plan - '||'"; '||
    'subgraph tree { bgcolor="#808080"; '||
    '0 [shape="rectangle" style="rounded, filled", color="#eb684b", penwidth="2", fillcolor="#ffffff", height="0.3", width="1.1", label="Result"];'
  union all select 2, row_number() over(order by p.id), -- 2. Nodes for operators
    ...
    case when p.operator = 'Aggregate' and filter_predicate is not null then icon.filter else '' end || -- index filter - orange 'F'
    ...
    case when p.index_name is not null then '<tr><td bgcolor="#c4ffca" align="left" colspan="2">' || icon.idx || -- index access - green 'I'
      case when p.operator = 'Index Only Scan' and filter_predicate is not null then '<font point-size="4" color="red"> </font>' || icon.filter else '' end || -- index filter - orange 'F'
    ...
    case when p.table_name is not null then
      ...
      case when p.filter_predicate is not null then '<font point-size="4" color="##ff8c00"> </font>' || icon.heap else '' end || -- heap filter - orange 'F'
    ...
    cross join cost_mf
    cross join icon
  union all select 5, row_number() over(order by p.id), -- 4. Streams
      p.id||'->'||coalesce(p.parent_id, 0)||'[color="#eb684b" label="\l '||
--      p.rows||
      to_char(p.rows / rows_mf.divisor, tformat)||rows_mf.append ||
      ' row'||case when p.rows = 1 then '' else 's' end||'"];'
    from p
    cross join rows_mf
