...
public class Example15 {
  ...
  public static void transferNoTransaction(final Integer from, final Integer to, final Integer amount)
    ...
    BranchVO b1 = BranchDAO.select(from);
    BranchVO b2 = BranchDAO.select(to);

    b1.setCurrentCash(b1.getCurrentCash() - amount);
    BranchDAO.update(b1); // updates branch #1

    b2.setCurrentCash(b2.getCurrentCash() + amount);
    BranchDAO.update(b2); // updates branch #2
   ...
   * 
   * This is a very advanced topic. You rarely will use this functionality in
   ...
   * 
   * In the example both transactions are safe. If there's a failure between the
   * updates the uncommitted transactions are rolled back and no changes are
   * performed on their behalf. The database reverts to the initial consistent
   * state. Committed transactions are not reverted.
   * 
   * There's no lost cash under hardware or software failures.
   */
  public static void transferInterlacedTransactions(final Integer from1, final Integer to1, final Integer amount1,
    ...
    try {
      ...
      BranchVO b1 = BranchDAO.select(sqlSession1, from1);
      BranchVO b2 = BranchDAO.select(sqlSession1, to1);

      BranchVO b3 = BranchDAO.select(sqlSession2, from2);
      BranchVO b4 = BranchDAO.select(sqlSession2, to2);

      b1.setCurrentCash(b1.getCurrentCash() - amount1);
      BranchDAO.update(sqlSession1, b1); // updates branch #1

      b3.setCurrentCash(b3.getCurrentCash() - amount2);
      BranchDAO.update(sqlSession2, b3); // updates branch #3

      b2.setCurrentCash(b2.getCurrentCash() + amount1);
      BranchDAO.update(sqlSession1, b2); // updates branch #2

      sqlSession1.commit(); // first commit

      b4.setCurrentCash(b4.getCurrentCash() + amount2);
      BranchDAO.update(sqlSession2, b4); // updates branch #4

      sqlSession2.commit(); // second commit

    } finally { // Always free resources in the finally clause!
      // In the absence of commit a SqlSession.close() call automatically rolls
      // back.
      try {
        sqlSession1.close();
      } finally {
