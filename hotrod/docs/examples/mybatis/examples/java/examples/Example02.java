package examples;

import java.sql.Date;
import java.sql.SQLException;
import java.sql.Timestamp;

import daos.BranchDAO;
import daos.ClientDAO;
import daos.DailyReportDAO;
import daos.VehicleDAO;
import daos.VisitDAO;

/**
 * Insert Using Sequences and Identity PKs
 * 
 * @author valarcon
 * 
 */
public class Example02 {

  public static void main(String[] args) throws SQLException {

    System.out.println("=== Running Example 02 - Insert Using Sequences and Identity PKs ===");

    int rows;

    // 1. Insert into a table with identity-generated PK

    VehicleDAO v = new VehicleDAO();
    // v.setId(...); // ignored: the DB will generate the PK
    v.setBrand("Skoda");
    v.setModel("Octavia");
    v.setType("CAR");
    v.setVin("VN4408");
    v.setEngineNumber("EN9401");
    v.setMileage(12100);
    v.setPurchasedOn(Date.valueOf("2017-02-17"));
    v.setBranchId(102);
    v.setListPrice(22400);
    v.setSold(false);
    rows = v.insert();
    System.out.println("1. Vehicle inserted using identity. ID=" + v.getId() + ". Rows inserted=" + rows);

    // 2. Insert into a table with sequence-generated PK

    ClientDAO c = new ClientDAO();
    // c.setId(...); // ignored: the PK value will be generated by the sequence
    c.setCreatedAt(Timestamp.valueOf("2017-02-22 18:15:21"));
    c.setName("Lee Van Cliff");
    c.setState("NM");
    c.setDriversLicense("TB1583-4530-KV");
    c.setReferredById(null);
    c.setTotalPurchased(0L);
    c.setVip(false);
    rows = c.insert();
    System.out.println("2. Client inserted using sequence. ID=" + c.getId() + ". Rows inserted=" + rows);

    // 3.a) Insert using into a table with optional identity PK (not specified)

    BranchDAO b = new BranchDAO();
    b.setId(null); // null: the DB will generate the PK
    b.setName("Wichita");
    rows = b.insert();
    System.out.println("3.a) Branch inserted (identity PK not specified). ID=" + b.getId() + ". Rows inserted=" + rows);

    // 3.b) Insert using into a table with optional identity PK (specified)

    b = new BranchDAO();
    b.setId(144); // specified value: this value will be used for the PK
    b.setName("Cincinnati");
    rows = b.insert();
    System.out.println("3.b) Branch inserted (identity PK specified). ID=" + b.getId() + ". Rows inserted=" + rows);

    // 4. Insert into table with PK, but no auto-generation

    DailyReportDAO r = new DailyReportDAO();
    r.setBranchId(104);
    r.setReportDate(Date.valueOf("2017-02-28"));
    r.setTotalSold(524850L);
    r.insert();
    System.out.println("4. Daily Report added. Rows inserted=" + rows);

    // 5. Insert into a table with no PK

    VisitDAO vi = new VisitDAO();
    vi.setRecordedAt(Timestamp.valueOf("2017-03-01 11:30:00"));
    vi.setBranchId(105);
    vi.setNotes("This is a note of the visit");
    rows = vi.insert();
    System.out.println("5. Visit inserted. Rows inserted=" + rows);

    System.out.println("=== Example 02 Complete ===");

  }

}
