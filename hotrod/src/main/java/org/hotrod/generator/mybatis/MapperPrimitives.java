...
public class MapperPrimitives {
  ...
  private void writeInsert() throws IOException, UnresolvableDataTypeException, AutogeneratedKeysNotSupportedException {
    ...
    if (agcm != null && agcm.isIdentity() && agcm.useIdentityGeneratedKeys()) {

      println("  <insert id=\"" + this.getMapperIdInsert() + "\" useGeneratedKeys=\"true\" keyProperty=\""
          + agcm.getIdentifier().getJavaMemberIdentifier() + "\">");
    ...
    for (ColumnMetadata cm : this.metadata.getColumns()) {
      boolean includeColumnInInsert = agcm == null || !agcm.getColumnMetadata().equals(cm) || !agcm.isIdentity()
          || (agcm.isIdentity() && agcm.allowsSpecifiedValue());

      if (includeColumnInInsert) {
        lw.add(cm.renderSQLIdentifier());
        String jdbcType = ",jdbcType=" + cm.getType().getJDBCShortType();

        if (agcm != null && agcm.getColumnMetadata().equals(cm) && agcm.isIdentity() && agcm.allowsSpecifiedValue()) {
          lwp.add("\n       <if test=\"seqId == null\">default</if>" + "\n       <if test=\"seqId != null\">#{"
              + cm.getIdentifier().getJavaMemberIdentifier() + jdbcType + "}</if>" + "\n       ");
        } else {
