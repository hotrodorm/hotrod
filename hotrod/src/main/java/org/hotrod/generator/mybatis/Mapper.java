...
public class Mapper {
  ...
  private void writeInsert() throws ControlledException, IOException, SequencesNotSupportedException,
    ...
    writeInsertVariant(false);
  }

  private void writeInsertVariant(final boolean byExample)
      throws IOException, SequencesNotSupportedException, IdentitiesPostFetchNotSupportedException {
    ...
    if (identities == 0) {
      if (sequences == 0) { // no sequences, no identities
        writeInsertWithoutAutoGeneratedColumn(byExample);
      } else { // sequences only
        if (this.adapter.getInsertIntegration().integratesSequences()) {
          writeInsertIntegrated(byExample, true, false);
        } else {
          writeInsertUsingSequencesPreFetch(byExample);
        }
      }
    } else {
      if (sequences == 0) { // identities only
        if (this.adapter.getInsertIntegration().integratesIdentities()) {
          writeInsertIntegrated(byExample, false, true);
        } else {
          writeInsertUsingIdentities(byExample, true);
        }
        ...
        if (integratesSequences && integratesIdentities) {
          writeInsertIntegrated(byExample, true, true);
          ...
          writeSequencesPreFetch();
          writeInsertUsingIdentities(byExample, false);
          writeIdentitiesPostFetch();
        }
  ...
  private void appendSequenceColumn(final boolean byExample, final ListWriter columns, final ListWriter values,
      final ColumnMetadata cm) throws SequencesNotSupportedException {
    columns.add("<if test=\"true\">, " + SUtils.escapeXmlBody(cm.renderSQLIdentifier()) + "</if>");
    values.add("<if test=\"true\">, " + this.adapter.renderInlineSequenceOnInsert(cm) + "</if>");
  }

  private void renderInsert(final boolean byExample, final ListWriter columns, final ListWriter values,
      final ListWriter queryColumns) throws IOException {
  ...
  private void writeInsertWithoutAutoGeneratedColumn(final boolean byExample) throws IOException {
    if (this.isTable()) {
      ...
      println("  <!-- insert " + (byExample ? "by example " : "") + "(view) -->");
    }
    println();
    ...
    renderInsert(byExample, columns, values, queryColumns);

    println("  </insert>");
    println();
  }

  private void writeInsertIntegrated(final boolean byExample, final boolean retrieveSequences,
      final boolean retrieveIdentities) throws IOException, SequencesNotSupportedException {
    ...
    println("  <!-- insert " + (byExample ? "by example " : "") + "(" + msg.toString() + ") -->");
    println();
    ...
    for (ColumnMetadata cm : this.metadata.getColumns()) {
      ...
      if (retrieveSequences && cm.getSequence() != null) {
        ...
        appendSequenceColumn(byExample, columns, values, cm);
  ...
  private void writeInsertUsingSequencesPreFetch(final boolean byExample)
      throws IOException, SequencesNotSupportedException {
    println("  <!-- insert " + (byExample ? "by example " : "") + "(using sequences prefetch) -->");
    println();
  ...
  private void writeInsertUsingIdentities(final boolean byExample, final boolean postFetch)
      throws IOException, IdentitiesPostFetchNotSupportedException {
    if (postFetch) {
      println("  <!-- insert " + (byExample ? "by example " : "") + "(using identities post fetch) -->");
    } else {
      println("  <!-- insert " + (byExample ? "by example " : "") + "-->");
    }
    println();
  ...
  // === Insert by Example (tables & views) ===
