...
public class DAOPrimitives {
  ...
  private void writeVOClass() throws IOException, UnresolvableDataTypeException {
    ...
    for (ColumnMetadata cm : this.ds.getColumns()) {
      ConverterTag cvt = cm.getConverter();
      if (cvt != null) {
        println("    private " + cvt.getJavaIntermediateType() + " " + cm.getIdentifier().getJavaMemberIdentifier()
            + " = null;");
      } else {
    ...
    for (ColumnMetadata cm : this.ds.getColumns()) {
      ...
      if (cvt != null) {
        ...
        println("    public void " + cm.getIdentifier().getSetter() + "(final " + type.getJavaClassName() + " " + m
            + ") {");
        println("      this." + m + " = " + m + ";");
        println("    }");
      }
  ...
  private void writeInsert() throws IOException, UnresolvableDataTypeException {
    ...
    if (hasPK) {
      if (pk.getColumns().size() == 1) {
        ...
        isSequence = !isIdentity && (pk.getColumns().get(0).getSequence() != null);
    ...
    if (scenario == SCENARIO_PK_AUTOGENERATED) {
      ...
      if (isSequence) {
        ...
        insertValues = pk.getColumns().get(0).getCanonicalSequence() + ".nextval," + insertValues;
      }
    ...
    if (scenario == SCENARIO_PK_AUTOGENERATED) {
      println("          PreparedStatement pst = con.prepareStatement(sql, new String[] { \""
          + pk.getColumns().get(0).getIdentifier().getSQLIdentifier() + "\" });");
  ...
  private void writeVOCloneMethods() throws IOException, UnresolvableDataTypeException {
    ...
    for (ColumnMetadata cm : this.ds.getColumns()) {
      println("    target." + cm.getIdentifier().getSetter() + "(this." + cm.getIdentifier().getGetter() + "());");
    }
    println();
